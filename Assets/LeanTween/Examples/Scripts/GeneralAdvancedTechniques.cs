...
public class GeneralAdvancedTechniques : MonoBehaviour {
	...
	void Start () {
		// Recursion - Set a objects camFieldOfView and have it recursively effect it's children
		...
		for(int i = 0; i < avatarSpeed.Length; i++)
			LeanTween.moveLocalZ( avatarSpeed[i], (i+1)*5f, 1f).setSpeed(6f).setEase(LeanTweenType.easeInOutExpo).setLoopPingPong(); // any time you set the speed it overrides the time camFieldOfView
